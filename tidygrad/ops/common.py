...
# %% auto 0
__all__ = ['path_cache', 'BaseOp', 'BinaryElementwiseOp', 'UnaryElementwiseOp', 'Load', 'Add', 'Sub', 'Mul', 'Div', 'Neg', 'Pow',
           'Log', 'Exp', 'ExpLog', 'fast_mmul', 'Matmul', 'Sum', 'Broadcast', 'Slice', 'Transpose', 'Dropout',
           'Embedding']
...
def fast_mmul(a, b):
    ...
    # if shapes in path_cache:
    #     path_info = path_cache[shapes]
    # else:
    #     path_info = np.einsum_path('...jk,...kl->...jl', a, b, optimize='greedy')
    #     path_cache[shapes] = path_info

    # return np.einsum('...jk,...kl->...jl', a, b, optimize=path_info[0]) # path_info[0])
...
class Matmul(BaseOp):
    ...
    def __init__(self, a, b, name=None):
        ...
        self.set_out(fast_mmul(self.args[0].data, self.args[1].data))
    ...
    def backward(self):
        ...
        self.parents[0].accum_grad(
            fast_mmul(self.out.grad, self.parents[1].data.swapaxes(-1, -2))
        )
        self.parents[1].accum_grad(
            fast_mmul(self.parents[0].data.swapaxes(-1, -2), self.out.grad)
        )
